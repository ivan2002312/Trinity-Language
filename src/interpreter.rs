...
impl Interpreter {
    ...
    pub fn eval(&mut self, e: &Expression) -> Result<Value, String> {
        match e {
            ...
            Expression::Identifier(n, _) => { if n == "len" { return Ok(Value::Int(0)); } self.get(n) },
            ...
            Expression::Call(name, args, _) => {
                ...
                match name.as_str() {
                    "read_line" => { let mut s = String::new(); std::io::stdin().read_line(&mut s).ok(); return Ok(Value::String(s.trim().to_string())); } "builtin_len" => { let s = av.get(0).map(|v| format!("{}",v)).unwrap_or_default(); return Ok(Value::Int(s.len() as i64)); } "print" | "println" => { for a in &av { print!("{}", a); } if name == "println" { println!(); } return Ok(Value::Void); }
