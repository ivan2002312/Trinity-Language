...
fn compile_to_exe(module: &ast::Module, source: &str, input: &Path, base_dir: &Path) {
    ...
    for imp in &module.imports {
        for c in &[base_dir.join(format!("{}.trm",imp)), exe_dir().join("modules").join(imp).join(format!("{}.trm",imp))] {
            if c.exists() { 
...
fn exe_dir() -> PathBuf { std::env::current_exe().unwrap_or(PathBuf::from(".")).parent().unwrap_or(Path::new(".")).to_path_buf() }
fn install_module(name: &str) { println!("Install: {} from https://github.com/ivan2002312/Trinity-Module", name); }
fn search_modules(q: &str) { println!("Search: {} at https://github.com/ivan2002312/Trinity-Module", if q.is_empty() { "all" } else { q }); }
fn list_modules() { let d = exe_dir().join("modules"); if d.exists() { for e in fs::read_dir(&d).unwrap().flatten() { println!("  {}", e.file_name().to_string_lossy()); } } }
